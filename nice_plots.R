...
myplot <- xyplot(Late ~ Time | StopName, stop_plot, type = "l", par.settings = my.theme, 
                 xlab = "Time", 
                 ylab = "Delay",
                 panel =function(x,y,...){ 
                   panel.xyplot(x,y,...);
...
for (i in 2:length(route))
{
  ...
  print(cors[i-1] <- cor_mat[stop,stop1])
...
matplot(x = c(2:length(route)), y = cors,  type = "l",
        lty = "solid",  lwd = 2,
        xlab = "Location", ylab = 'Correlation of delays between two nighebouring stops',
        xaxt = 'n', col = data_colors[length(route)+1])
axis(1, c(1:length(route)), route,  tck = 0, las = 1)
for(i in 1:(length(route)-1))
{
  xpos <- i+1
  ypos <- cors[i]
  text(label=format(round(ypos, 2), nsmall = 2), x = xpos, y = ypos, col = "black")
}
return(myplot)
}